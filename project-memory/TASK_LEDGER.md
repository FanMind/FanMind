...
- Action: add universal `Nachrichten|Follow-ups|Kontaktwissen` tabs, move profile/tags into knowledge, add provenance-bound fan-analysis read/prepared server action, add per-fan/today Follow-up queries and navigation, and replace the splash asset with a square `FM`-over-`FanMind` composition. Two review passes then tightened the implementation: hide generation until Production capability activation; require complete provenance at render time; preserve only a missing-column-compatible legacy Web read with null provenance; separate initial and post-create Follow-up errors; use exact paginated today counts with semantic priority; and map typed API plus inactive-Workspace failures to semantic statuses.
- Result: initial head and first corrective head each passed all nine CI gates but were intentionally not merged because their respective reviews found six and then four valid boundary/usability cases. The final grouped correction and focused checks are complete locally; final exact-head evidence remains pending.
- Evidence: FM-CR-005; active lock/receipt; PR #1025; superseded green heads `2feba6f63d611a8461e2a9bb3402147f7fff8dd5` and `c7226cab1a991a514f3fd9d19e58b00e250135a4`; ten review threads addressed rather than bypassed.
- Next step: publish the final corrected head, resolve the four new threads, require green final exact-head CI, merge and queue exactly one preview Android build.
