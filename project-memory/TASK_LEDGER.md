...
- Next step: separately authorize and run the checksum-bound protected-Staging apply. After it succeeds, rerun read-only verification, then separately authorize the rollback-only PostgreSQL acceptance. Real provider/device delivery remains later and separately gated.
