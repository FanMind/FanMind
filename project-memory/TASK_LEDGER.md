...
- Next step: publish the safe diagnostic, require exact-main CI, then rerun the already authorized Staging apply to identify and repair the precise non-secret failure class. Keep rollback-only acceptance and scheduling blocked behind their own explicit approvals.
...
- Next step: preserve the exact Production AAB and never rebuild it merely to continue portal work. When FanMind is ready for the Gerhard handoff, enroll at least 12 approved testers for at least 14 days and complete the private 19-check/Recovery record plus real screenshots from the Play-installed Alpha build before requesting Production access. Do not start an iOS build before Phase 8.
