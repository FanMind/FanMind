...
- Completed so far: source/JWT/revision/PDF foundation is accepted; confirmed-chat validator/persistence/disclosure/delete verification is merged through #1167. Controlled rollout hardening converged through #1168/#1171/#1172. PR #1173 established the target-aware lifecycle (Staging `installed`, Production/unknown `preinstall`) and disabled generic APPLY; PR #1174 corrected its gitless isolated-release test regression. Exact main `c119e0eb82e0643fc676afb7725d7518011e1286` deployed successfully in run `36056745406`; Browser E2E, CodeQL, God Mode and Final Go-Live Readiness passed. PR #1175 then added the repository-only protected read-only Staging VERIFY entrypoint; final head `398abaa709ae4e526e1bd1d1d16df832b6ade815` resolved its initial P1/P2 findings and merged as exact main `6e43bc6a86d73bffec203f7af2270002255a3008`. No confirmed-chat schema Apply or runtime learning activation occurred.
- Exact next step: the #1175 source-control package is consumed and must not be rebuilt. Actual Staging deployment/VERIFY remains a separate protected evidence action and any APPLY requires a separately reviewed protected path plus action-time authorization. Recompute the Builder Manager before any Creator continuation. Only if `NBA-CREATOR-INTELLIGENCE` is admitted to the current SAFE READY SET may a distinct repository-safe unaccepted Creator scope proceed; otherwise reconcile stale/terminal continuations or another manager-admitted scope and do not bypass worker/conflict limits.
## FM-SEC-002
- Date: 2026-09-10
- Status: VERIFIED
- Risk: R3
...
- Next step: owner reprioritized Web/Social after the accepted Admin-CRM login. Current bounded work is FM-CR-044 / PR #1137; then use the existing granted account in `/channels` for Facebook first and Instagram second. Real consent/App Review/permissions/webhook/inbound/isolation/duplicate/revocation/reconnect evidence remains required. WhatsApp stays dormant and separately gated.
...
- Exact next step: #1115 is reviewed/deployed on 1bdf5583 (final receipt 5652891341). Owner confirms corrected modes, both protected PATH lists and byte-identical migrated registration without alternate credentials. Finish the demonstrated opaque-Pipelines-path/migration-binding correction through current-head review/CI/normal rollout, then independent reference and installed boot proof. Both co-located runners need fresh idle/recovery checks before the requested controlled reboot. Overall reboot acceptance remains open.
...
- Exclusions: no automatic email confirmation or Stripe/payment/Tax/provider/Mobile/Restore changes. The separately owner-authorized exact Production Admin-CRM schema rollout is complete and must not be repeated.
- Acceptance: FM-CR-041 and LOCK-FM-REG-003-ADMIN-CRM-ACCESS-20260915.
- Evidence: PR #1134 current-head CI/review passed and merged as `630aef3ccb53fed9b46284cb1d4bf1825a57687e`; Production deploy `35431328695` passed. Exact R4 migration applied once as `20260919081945 admin_crm_access` with independent RPC/ACL/RLS postflight. Receipt: `project-memory/receipts/FM-REG-003-ADMIN-CRM-PRODUCTION-APPLY-20260919.md`.
- Historical review continuation: PR #1132 findings led to the transactional service-role-only migration. That source-only/unapplied state was superseded by reviewed PR #1134 and the separately authorized Production apply above; do not repeat the old pending steps.
- Historical second-review continuation: the restrictive entitlement boundary, generic Billing/Test/Daily rejection, exact owner resolution and checksum-pinned runbook were incorporated into #1134. The old `migration remains unapplied` and Mobile-CI-red statements are superseded by the completed Production apply and accepted FM-CR-042 source/CI patch.
- 2026-09-18 historical review correction: rollout-order-independent Creator entitlement enforcement, inactive-route projection, temporary-expiry-bounded Mobile cache and foreign-membership rejection were implemented and later accepted in #1134.
- 2026-09-19 Production closeout: PR #1134 merged as `630aef3ccb53fed9b46284cb1d4bf1825a57687e`; deploy `35431328695` passed. Explicit owner authorization was consumed for exact migration SHA-256 `7d1201fc5b45b571d2944b301eb1f5f197ea4f25ad643c8e8010d9d0ba3c1efd`, recorded as `20260919081945 admin_crm_access`; postflight proves service-role-only mutation and 19 restrictive policies. Synthetic lifecycle acceptance was accidentally skipped before the first real grant; no further real grants until that gap is closed.
- 2026-09-19 real browser closeout: #1136 final head `40da6a14e05ee977d6ebadc94d34f9db648a8343` passed current-head checks/review, merged as `a5fb5e133d3ef49f745bed6d3e599d24d73bb493`, and Production deploy `35434847795` passed. The owner then confirmed the same already-granted account reaches CRM rather than `/billing/start`. FM-CR-043 is PRODUCTION_CONFIRMED. FM-REG-003 remains IN_PROGRESS only for the separately missing synthetic permanent -> future temporary -> blocked -> login/direct-read lifecycle; no further real grants until that acceptance is completed.
