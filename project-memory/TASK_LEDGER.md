...
- Completed so far: source/JWT/revision/PDF foundation is accepted; confirmed-chat validator/persistence/disclosure/delete verification is merged through #1167. Controlled rollout hardening converged through #1168/#1171/#1172. PR #1173 established the target-aware lifecycle (Staging `installed`, Production/unknown `preinstall`) and disabled generic APPLY; PR #1174 corrected its gitless isolated-release test regression. Exact main `c119e0eb82e0643fc676afb7725d7518011e1286` deployed successfully in run `36056745406`; Browser E2E, CodeQL, God Mode and Final Go-Live Readiness passed. Read-only Production Audit independently verified that exact runtime and remains red only for the pre-existing backup-freshness Operations loop. No confirmed-chat schema Apply or runtime learning activation occurred.
- Exact next step: publish the repository-only VERIFY control for confirmed-chat learning on isolated Staging. The control is exact-main/exact-commit, protected-`staging`, TLS/passfile-bound and read-only; it exposes no APPLY mode. After source convergence, actual Staging deployment and a fresh target VERIFY remain separate protected evidence steps. Any APPLY requires a later separately reviewed protected path plus action-time authorization.
## FM-SEC-002
- Date: 2026-09-10
- Status: VERIFIED
- Risk: R3
