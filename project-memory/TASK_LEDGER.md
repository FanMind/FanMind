...
- Additional resource evidence: protected read-only Push resource run `33797049971` / job `100787171346` passed exact commit/target separation and the pinned registration checksum, then failed closed before database inspection because the five dedicated Mobile Push Staging variables are unset. An eligible isolated non-demo Workspace/Owner/Member tuple and the already verified EAS project binding are available, but were not written to GitHub.
- Next step: configure the five dedicated protected Staging variables and rerun resource readiness. Only after it is green separately authorize the checksum-bound registration/ledger applies, repeat read-only verification, and run the rollback-only PostgreSQL acceptances. Real provider/device delivery remains later and separately gated.
