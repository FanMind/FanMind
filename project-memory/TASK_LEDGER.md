...
- Checks so far: first PR head passed lint, Operations tests and the real PostgreSQL 17 authorization roundtrip; Project Memory Guard correctly failed because the code-only head lacked a memory update. This receipt and the accepted database-phase receipt correct that omission; final exact-head CI/review remains pending.
- Result status: IN_PROGRESS until exact-head CI/review and merge. Overall `FM-RST-001` remains PARTIAL; this code fix does not authorize or repeat a Restore.
- Open follow-up: after #1075 merge, close issue #944's permanent-helper loop and reconcile the exact next state-machine boundary without inferring Storage/server-config/disposable-target acceptance.
- Work lock released: no; release only after exact-head merge/evidence reconciliation.
