...
- Next step: resource readiness is green. Separately authorize the checksum-bound registration/ledger applies, then repeat read-only verification and run the rollback-only PostgreSQL acceptances. Real provider/device delivery remains later and separately gated.
