...
- Action: add universal `Nachrichten|Follow-ups|Kontaktwissen` tabs, move profile/tags into knowledge, add provenance-bound fan-analysis read/prepared server action, add per-fan/today Follow-up queries and navigation, and replace the splash asset with a square `FM`-over-`FanMind` composition. Four review passes then tightened the implementation: hide generation until Production capability activation; require complete provenance on Mobile and Web; permit legacy Web reads only after all new columns are individually proven absent; separate initial/post-create/dashboard/knowledge errors; select semantic priority groups before the exact paginated day-list cap with stable `created_at`/`id` boundaries and a last legacy fallback group; expose exact/truncated per-contact results; cap fallback-only analysis confidence low; and map typed API plus inactive-Workspace failures to semantic statuses.
- Result: four successive heads passed all nine CI gates but were intentionally not merged because their reviews found six, four, six and three valid boundary/usability cases. The fourth grouped correction passes the complete local regression matrix; final exact-head evidence remains pending.
- Evidence: FM-CR-005; active lock/receipt; PR #1025; superseded green heads `2feba6f63d611a8461e2a9bb3402147f7fff8dd5`, `c7226cab1a991a514f3fd9d19e58b00e250135a4`, `67e02276ede02bb919088f3aa61e0e1343be52e0` and `cca0b7e2cc650b886a0a10653d49a15536e27d0a`; nineteen review threads addressed rather than bypassed.
- Next step: publish the final corrected head, resolve the three new threads, require green final exact-head CI, merge and queue exactly one preview Android build.
