...
| ChatAdmin Manual Flow | FM-CHATADMIN-002 | OWNER_ACTION_REQUIRED | DB/RLS ACCEPT intentionally emitted `CHAT_ADMIN_ACCEPTANCE_MANUAL_FLOW=OPEN` | separately authorize and execute real Staging application-layer flow with synthetic data: Character -> manual Fan message -> exactly three revision-bound suggestions -> Copy -> manual-send handoff; cleanup required; no provider send |
