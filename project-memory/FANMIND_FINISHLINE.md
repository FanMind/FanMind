...
| ChatAdmin Staging ACCEPT | FM-CHATADMIN-002 | OWNER_ACTION_REQUIRED | APPLY/postflight VERIFIED and persistent ChatAdmin rows remain zero | fresh current-main/target binding; protected `ACCEPT` + `run-chat-admin-acceptance`; rollback-only synthetic DB/RLS proof; manual application flow remains separate |
