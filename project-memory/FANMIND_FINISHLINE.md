...
| Restore | FM-RST-001 | PARTIAL | ACL/Owner recovery contract, PG17 roundtrip, Schema-2 Full Backup/checksum, isolated host; real isolated PostgreSQL-17 Restore committed in workflow `33178878764` / job `98874745740`; one-shot ACL completion `5453727223` applied exactly eight missing schema-USAGE grants, matched the projected authorization fingerprint, passed core `5|5|5|5` and plaintext cleanup; PR #1075 permanently fixed the target-only principal projection and #944 is closed | reconcile every receipt-bound `DB_RESTORED -> DB_POSTCHECKED` predicate without repeating Restore; then Storage -> config -> disposable-target cleanup -> independent countercheck -> final aggregate acceptance |
