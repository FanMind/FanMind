...
## FM-FAIL-023
- Date: 2026-09-12
- Status: DOCUMENTED_LOCAL_LIMITATION
- Area: FM-OPS-001 / FM-CR-036 runner executable proof, PR #1112.
- Attempt: native executable identity regression in the managed workspace.
...
- Configuration countercheck: the same run still received one trailing control character on the Owner-ID, and current read-only editor inspection at `2026-09-26T15:08:32Z` still reports outer whitespace. The earlier UI correction claim is not accepted; exact receipt/absence reconciliation and protected whitespace-free canonical-value repair are both required before retry.
