...
- Current side state: `RECONCILIATION_REQUIRED` only for the permanent helper correction and exact classification of later transitions. PR #1075 keeps raw source capture unchanged, proves the connected Restore principal is the unique external login/superuser, then excludes exactly that principal from projected target comparisons.
...
- Exact next step: merge #1075 after exact-head CI/review, close issue #944's permanent-helper loop, and continue from `DB_RESTORED -> DB_POSTCHECKED` by evidence reconciliation or read-only proof only. No new database Restore, target reset, JIT reuse or automatic retry is authorized.
