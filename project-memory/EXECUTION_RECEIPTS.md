...
- Recovery boundary: stop new fixture dispatches if the readback or bound owner/workspace countercheck becomes contradictory. Re-read this one Staging variable; the pre-edit state was exactly the same UUID plus one trailing newline. Only if reverting this formatting correction is necessary, restore that exact prior string through the same GitHub variable editor and verify it; never substitute another identity, rotate credentials or alter protection rules. Reconcile the contradiction before resuming acceptance. No rollback is currently needed.
