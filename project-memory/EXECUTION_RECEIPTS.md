...
- Branch/PR: `feat/mobile-fan-sections-analysis-followups-splash-20260829` / #1025; superseded reviewed green heads `2feba6f63d611a8461e2a9bb3402147f7fff8dd5`, `c7226cab1a991a514f3fd9d19e58b00e250135a4`, `67e02276ede02bb919088f3aa61e0e1343be52e0` and `cca0b7e2cc650b886a0a10653d49a15536e27d0a`, final corrective exact head pending
...
- Changes made: three universal fan sections, one-line identifier, moved profile/tags, provenance-bound stored analysis display with the inactive Production generation control visibly marked `In Vorbereitung`, per-fan/today Follow-up reads/navigation, exact paged day count with explicit truncation and semantic priority sorting, and square splash; docs/tests updated. Four remote review passes found nineteen valid boundary/error/count/compatibility defects. The grouped corrections add complete Mobile/Web provenance gating, whole-legacy-schema-only Web compatibility with null provenance, initial/post-create/dashboard/knowledge error visibility, typed API/Workspace status handling, priority-before-cap selection with stable/fallback pagination, exact per-contact truncation and low-confidence fallback-only analysis.
- Checks/tests: Initial implementation passed Mobile check including TypeScript, Expo Doctor 20/20, Store/boundary and 33-file native prebuild; Android and iOS Expo exports; focused Mobile/security 32/32 plus the post-hardening 30/30 authorization set; complete Operations 1055/1055; root lint/build including `/api/ai/fan-analysis`; and Project Memory status/quality/truth/drift. All four superseded reviewed PR heads `2feba6f63d611a8461e2a9bb3402147f7fff8dd5`, `c7226cab1a991a514f3fd9d19e58b00e250135a4`, `67e02276ede02bb919088f3aa61e0e1343be52e0` and `cca0b7e2cc650b886a0a10653d49a15536e27d0a` passed all nine remote gates but were deliberately not merged after their respective reviews. The newest grouped correction passes focused Mobile regression, Mobile and root TypeScript, the full Mobile check including Expo Doctor 20/20/native prebuild, Android and iOS Expo exports, complete Operations 1055/1055, root lint and a clean-cache production build, all Project Memory status/quality/truth/drift/freshness/milestone/V8/NBA checks, Product Truth and `git diff --check`; replacement exact-head validation remains pending.
- Final diff counterchecked: no; pending final exact head.
- Regression/security countercheck: Initial local controls passed for explicit Workspace/contact/date filters and limits, RLS read boundary, Owner-only server action, Member read-only UI, bounded JSON input and absence of schema, demo-row, provider, Production, automatic-send or client-secret changes. Review then required a stricter fail-closed result: Mobile and Web analysis require valid non-null source period, confidence and review status at render time; the unavailable Production analysis action is not exposed as active; the server fallback requires explicit missing-column errors for every provenance/review column and returns null provenance; API failure classes retain meaningful HTTP status; Follow-up initial/refresh/dashboard/count/ordering states are explicit; and knowledge errors cannot appear as a valid empty state.
