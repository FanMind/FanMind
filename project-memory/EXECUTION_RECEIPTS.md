...
- Changes made: added the RLS-protected bounded message query, newest-first read-only history UI, explicit refresh, message-specific empty/error state, no-auto-send state, types, docs, tests and same-change Source-of-Truth/Project-Memory drift reconciliation. No database/provider/Production mutation.
- Checks/tests: local Mobile check, Android/iOS exports, truth, lint, focused 40/40 Mobile/security tests, full 1052/1052 operations tests and `git diff --check` passed before the Project Memory amendment. Project Memory checks and exact-head remote gates are pending rerun.
