...
- Active focus: `FM-RST-001` remains `PARTIAL` at `TARGET_COMPATIBLE`; side state `RECONCILIATION_REQUIRED` after controller `45054c41...` timed out on its first SSH connection before JIT/dispatch/database access. Repository-only lock `LOCK-FM-RST-001-SSH-TIMEOUT-RECONCILIATION-20260826` is active.
