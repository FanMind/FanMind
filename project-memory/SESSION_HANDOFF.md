...
- Mobile boundary: FM-MOB-003 is `VERIFIED_NOT_ACCEPTED`; the work lock is released. Supabase recovery redirect, broader Recovery/Purge acceptance, iOS/TestFlight, push and Store controls remain open under FM-MOB-001. A device failure reopens the exact runtime observation only; it does not authorize schema/RLS changes or duplicate demo data.
