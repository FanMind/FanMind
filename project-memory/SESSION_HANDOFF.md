...
- Active focus: `FM-RST-001` remains `PARTIAL` at `TARGET_COMPATIBLE`; side state `RECONCILIATION_REQUIRED` after controller `45054c41...` timed out before JIT/dispatch/database access. Parallel `FM-SEC-001` protected read-only run `32997946812` is complete: preflight/postflight audits passed and the exact Production pre-state returned `hardening_not_ready`. PR #1008 and its evidence lock remain active only until publication/countercheck/merge; no second dispatch.
- Start here: run the complete Project Memory/Restore preflight, inspect current GitHub and Production `main` `5cb9c193e262f8939b5fc0c700fce154dde616e6`, deploy run `32996396550` job `98266724400`, protected verify `32997946812` job `98271985321`, PR #1008, issue #944 comments `5385992305`/`5386014235`/`5428771745`, issue #982 comments `5428919200`/`5428996454`, FM-EV-017/FM-EV-018/FM-EV-019/FM-EV-020 and the owner-supplied timeout output. Current GitHub/runtime evidence overrides older memory prose.
