...
- Active focus: `FM-RST-001` remains `PARTIAL` at `TARGET_COMPATIBLE`; side state `RECONCILIATION_REQUIRED` after controller `45054c41...` timed out on its first SSH connection before JIT/dispatch/database access. PR #1005 reconciled that failure. The parallel read-only Supabase refresh then passed every exact-head gate in #1006 and merged as `78333aae9d075a67a2d550a266d24cb8b9f443a4`; both repository-only locks are released.
- Start here: run the complete Project Memory/Restore preflight, inspect current GitHub `main` `78333aae9d075a67a2d550a266d24cb8b9f443a4`, issue #944 comments `5385992305`/`5386014235`/`5428771745`, issue #982 comment `5428919200`, FM-EV-017/FM-EV-018/FM-EV-019 and the owner-supplied timeout output. Current GitHub/runtime evidence overrides older memory prose.
...
- Security deviation: both Supabase targets are currently `ACTIVE_HEALTHY`; FM-EV-019/#1006 reconfirmed the Production/Staging findings without provider drift or mutation. Remediation in issue #982 remains separately gated and forbidden from this Restore path.
