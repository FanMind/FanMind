...
- Boundary: the published Production workflow remains verify-only until a fresh commit-/target-/time-bound readiness receipt exists. Daily provisioning and every checkout entry recheck the complete billing runtime gate; stale runtime locks use a bounded lease; disabled Daily is not advertised on public legal surfaces. Production remains `BLOCK`; no Apply, runtime flag change, Admin enable, checkout or payment was performed.
- Review acceptance: Admin-off expires all identifiable open Daily Checkout Sessions and the shared creation boundary expires a concurrently created Daily session before returning its URL. Runtime lock cleanup requires the writer token. Invalid verify dispatches fail explicitly before protected Production access.
