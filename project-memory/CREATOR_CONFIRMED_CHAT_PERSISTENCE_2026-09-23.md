...
- Confirmation is service-route mediated: the exact stored text is authenticated-user-readable and grapheme-validated before the service-only RPC; the RPC then verifies exact text equality plus actor ownership and active processing. No service-role credential is exposed to the browser.
