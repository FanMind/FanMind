...
3. Resolve the current Stripe conformance review before commercial activation: at observation time the internal daily-test Checkout fixed `payment_method_types[]=card`, the endpoint pinned `2026-06-24.dahlia`, and server calls used raw REST instead of the current Stripe client pattern. FM-CR-009 locally corrects the productive code findings with SDK `22.4.0` and outbound `2026-07-29.dahlia` while deliberately retaining the observed older inbound endpoint as a separate provider-side migration gate; exact-head merge/closeout is pending. These findings never grant permission to change provider resources.
...
`FM-AI-001` remains `PARTIAL`. The old claims that the synthetic Staging resource or Plus/Ultra Test prices are missing are superseded. The productive Stripe code conformance correction is now locally counterchecked under FM-CR-009 and awaits exact-head closeout; provider-side inbound webhook migration remains separate. The genuine blockers are current post-ledger lifecycle acceptance, unapplied general Billing ledger/cutover, product/quality/cost evidence, provider-side webhook migration, Legal/Tax, runtime integration and explicit Production activation.
