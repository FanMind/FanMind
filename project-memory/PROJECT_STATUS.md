...
- PR #1193 and the environment-test correction #1194 are merged; exact main is `8ebb344633618de27c570734210c41ce69fc7721`. Continue branch `fix/staging-release-state-preservation-20260926` under LOCK-FM-CHATADMIN-MANUAL-FLOW-20260926; do not duplicate this scope.
...
- Current prerequisite: review/CI/merge the release-state preservation and bounded exact live recovery. First deploy `36247833422` exposed the fixed test defect after source copy; second `36248407036` failed before copy at capture preservation. The source copy had deleted generated release metadata before replacement. Recovery may only recreate an actually absent file from stable exact running Staging metadata, with complete unchanged proofs, explicit old SHA and billing freeze `preserve`; no default/reconstructed proof or Billing activation. Actual runtime remains unaccepted. Then deploy corrected reviewed main, execute synthetic flow/negatives, clean up and independently countercheck.
