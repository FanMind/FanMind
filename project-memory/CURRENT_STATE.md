## ChatAdmin deployed; existing-admin login-contract correction in progress — 2026-09-26
- FM-DEC-024 prioritizes ChatAdmin, then Creator Intelligence, existing free Admin-CRM access, its distinct synthetic lifecycle, Social and Sales. Accepted free access is preserved.
- ChatAdmin isolation and protected acceptance source PR #1193, Staging-aware test correction #1194 and release-state preservation/recovery #1196 are merged. Exact current main is `930ae196d20c0cd646b30a3d9a210269d694abeb`.
- Protected Staging deploy `36249685518` / job `108425269678` succeeded. `STAGING_RELEASE_STATE_RECOVERY=PASS` and `STAGING_RELEASE_RECOVERY_SECRETS_OUTPUT=0` prove bounded unchanged metadata recovery; all host Operations tests passed (1,579 total; 1,575 pass; 0 fail; 4 explicit skips), build/restart and release checks passed. Independent `/api/version` at `2026-09-26T14:49:02.035Z` confirms exact `930ae196` and runtimeEnvironment=staging.
- Actual manual-flow run `36249816690` / job `108425635786` stopped at `CHAT_ADMIN_MANUAL_FLOW_ERROR=fixture_credential` before reservation, recovery artifact, fixture mutation or browser/provider execution. Dedicated passfile removal succeeded. Existing secrets are present; no values were read or printed. The new validator incorrectly imposes a 16-character creation-style requirement on the pre-existing admin login, while established `admin-e2e-staging.yml` requires a supplied password and proves authentication/authority live.
- Bounded correction `fix/chatadmin-existing-admin-login-contract-20260926` aligns only existing-admin credential consumption, retaining synthetic-owner checks, exact admin identity and live admin-access proof. No password reset, account mutation or secret change. After current-head tests/review/CI/merge and exact Staging deployment, continue the actual manual flow.
- Independent post-failure Staging query at `2026-09-26T14:51:42.539013Z` confirms all four ChatAdmin table counts 0/0/0/0 and global chat_admin_reply usage 0. Manual gate remains IN_PROGRESS; FM-AUTH-CHATADMIN-MANUAL-FLOW-20260926 and its lock remain active. Historical schema APPLY and DB/RLS ACCEPT remain consumed.
