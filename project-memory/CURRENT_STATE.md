...
1. **FM-RST-001:** `DB_POSTCHECKED` is accepted and the private Storage artifact/receipt preparation is repository-ready; do not repeat the database Restore. After exact-head merge, require a new exact R4 authorization and distinct isolated non-Production target before implementing/dispatching any Storage upload. Server-config/disposable-target/final evidence remain later transitions.
