...
3. **FM-MOB-001:** preserve the accepted bounded FM-MOB-004 Android UI/runtime result and the exact Production AAB for `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`. Do not queue another Store build. Complete the real-device Recovery proof for the saved Supabase redirect and the private receipt-bound 19-check Android runbook separately; after Google completes developer-account review, confirm the phone number, create the Play app record and perform Data Safety/test-track/upload acceptance. iOS/TestFlight is Phase 8 and must not be started during the current finishline.
