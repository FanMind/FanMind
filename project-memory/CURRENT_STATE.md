...
- This supersedes every older active instruction below that says `DB_RESTORED` was not reached or asks for SSH reconciliation/new database-Restore authorization. Do not repeat run `33178878764`, job `98874745740` or the database Restore. PR #1075 permanently fixed the target-only login projection; issue #944 closed. PR #1077 merge `b3debab3483682a78d42046d77f0eeb5b6f409fb` retired stale SSH/new-Restore selectors. Receipt `FM-RST-001-DATABASE-POSTCHECK-ACCEPTED-20260907.md` maps exact evidence to every transition predicate and advances the accepted state to `DB_POSTCHECKED`. Overall `FM-RST-001` remains `PARTIAL` because Storage, server-config, disposable-target and final aggregate acceptance remain separate.
...
1. **FM-RST-001:** `DB_POSTCHECKED` is accepted from exact receipt-bound evidence; do not repeat the database Restore. Build and verify the missing bounded Storage-restore control path without dispatching it, then require separate exact authorization for any isolated Storage mutation. Server-config/disposable-target/final evidence remain later transitions.
