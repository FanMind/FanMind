...
- New bounded repository scope: controlled, unapplied `owned_workspace_ids` contract plus fail-closed processor persistence/resume logic. Dry-run remains readable; real execute must persist and read back every currently owned Workspace ID before Auth deletion. Resume after missing Auth must use only that stored inventory. Missing contract/inventory blocks completion.
