...
2. **Mobile — FM-MOB-001, R3:** signed Android preview and the bounded real-device UI/runtime observation are complete for FM-MOB-003/FM-MOB-004. PR #1028 merged the Android `1.0.0` Store control as `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`; protected Production readiness run `33316105624` / job `99269748215` passed, protected Store-build run `33316172583` / job `99269924756` completed exactly one commit-bound signed Android AAB with redacted receipt, Submit/Update disabled and cleanup successful, and `fanmind://reset-password` is saved in the exact Production Auth allowlist. On 2026-09-03 that AAB was published in the closed Google Play Alpha track for Germany, Austria and Switzerland. The deliberately deferred cohort of at least 12 opted-in testers for at least 14 days, the complete receipt-bound 19-check Android runbook/private validator, real Recovery flow, later Production-access request and applicable push/store acceptance remain open. `iOS-TestFlight` was moved by owner decision FM-DEC-009 to Phase 8 and no longer blocks this through-Phase-7 finishline; native iOS code/CI foundations remain intact.
...
3. **FM-MOB-001:** preserve the accepted bounded FM-MOB-004 Android UI/runtime result and the exact Production AAB for `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`; it is already published in the closed Alpha track. Do not queue another Store build. When FanMind is ready for the Gerhard handoff, enroll at least 12 approved testers for at least 14 days and complete the receipt-bound 19-check Android/Recovery proof plus real screenshots from the Play install before requesting Production access. iOS signing/build/TestFlight/device work remains Phase 8.
