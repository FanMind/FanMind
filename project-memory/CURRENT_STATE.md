...
2. **Mobile — FM-MOB-001, R3:** signed Android preview and the bounded real-device UI/runtime observation are complete for FM-MOB-003/FM-MOB-004. PR #1028 merged the Android `1.0.0` Store control as `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`; its exact signed AAB is published in the closed Google Play Alpha track for Germany, Austria and Switzerland, and `fanmind://reset-password` is saved in the exact Production Auth allowlist. FM-MOB-006 accepted the isolated-Staging atomic Push Delivery-Ledger gate on exact commit `18a6ad79cb72331b4daa41ee87dd2430a8ffd473`: apply `33867831888` / `101006621418` and rollback-only acceptance `33867922978` / `101006906941` passed with provider sending disabled, synthetic rows, complete rollback and cleanup. Real Push remains open because the published AAB predates the message handler; next is a separately reviewed handler-containing signed Android build and then separately authorized real-device/provider acceptance. The receipt-bound 19-check runbook, real Recovery flow and later Production-access request also remain open. The cohort of at least 12 opted-in testers for at least 14 days stays deferred until Gerhard handoff readiness. `iOS-TestFlight` remains Phase 8.
...
3. **FM-MOB-001:** preserve the accepted bounded UI/runtime result and published Alpha AAB. The atomic Push Delivery-Ledger isolated-Staging gate is accepted on `18a6ad79cb72331b4daa41ee87dd2430a8ffd473`, but that historical AAB predates the message handler. Next prepare exactly one separately reviewed signed Android build containing the merged handler; only afterwards and with separate authorization perform real provider/device Push acceptance. When FanMind is ready for the Gerhard handoff, enroll at least 12 approved testers for at least 14 days and complete the receipt-bound 19-check Android/Recovery proof before requesting Production access. iOS signing/build/TestFlight/device work remains Phase 8.
