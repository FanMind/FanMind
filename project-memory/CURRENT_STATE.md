...
2. **Mobile — FM-MOB-001, R3:** signed Android preview and the bounded real-device UI/runtime observation are complete for FM-MOB-003/FM-MOB-004; the complete receipt-bound 19-check Android runbook/private validator, redirect/recovery and applicable push/store acceptance remain open. `iOS-TestFlight` was moved by owner decision FM-DEC-009 to Phase 8 and no longer blocks this through-Phase-7 finishline; native iOS code/CI foundations remain intact.
...
3. **FM-MOB-001:** preserve the accepted bounded FM-MOB-004 Android UI/runtime result for exact merge `6a2f5b6c9bac1607ecc2ccae11c6ade3cb418522`; complete the private receipt-bound 19-check Android runbook/validator, redirect/recovery and applicable Push/Store controls separately. Do not queue another build merely to repeat the accepted UI checks. iOS/TestFlight is Phase 8 and must not be started during the current finishline.
