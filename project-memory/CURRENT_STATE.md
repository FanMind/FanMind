...
10. **Phase 8 Website AI — FM-WEB-001/FM-WEB-002, R3:** repository-side Website Chat now includes a dormant consent-bound human email handoff, complete existing CRM timeline linkage, a checksum-pinned atomic processing gate and separate exact-main protected controls for read-only Staging verification, independently confirmed apply and rollback-only synthetic acceptance. It is `IMPLEMENTED_NOT_VERIFIED`: no workflow was dispatched, the controlled SQL is unapplied, all installations remain disabled and real Staging acceptance, retention cleanup, AI dialogue, uncertainty escalation, email verification and approved delivery remain open.
