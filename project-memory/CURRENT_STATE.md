...
1. **FM-CREATOR-001:** the Builder Manager correction is accepted through #1180 / main `2c4954048d3ca359ac96978764b46e435854f0f4`, its governance lock is released, and fresh post-merge recomputation admits `NBA-CREATOR-INTELLIGENCE` as the repository-safe READY scope. Preserve all accepted #1175/confirmed-chat foundations; continue only the next bounded Creator engineering increment. No provider activation, auto-send, Mobile work, SQL APPLY or protected target mutation is implied.
2. **FM-REG-003:** the synthetic Admin-CRM lifecycle remains OWNER_ACTION_REQUIRED. FM-CR-043 remains PRODUCTION_CONFIRMED and the existing real 0-EUR account works; do not repeat registration, grant or Production DB apply. No additional real grants until the synthetic permanent -> future temporary -> blocked -> login/direct-read lifecycle is separately authorized and accepted.
3. **FM-CHATADMIN-002:** read-only VERIFY is consumed as `ABSENT` and God Mode v1 is merged/reconciled. The next ChatAdmin step is separately protected Staging APPLY under `FM-CHATADMIN-OWNER-APPLY-20260921`; it is OWNER_ACTION_REQUIRED and must not be dispatched autonomously. ACCEPT remains later.
