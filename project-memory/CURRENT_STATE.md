...
2. **FM-CHATADMIN-002:** read-only VERIFY and the owner-authorized Staging APPLY are consumed; the APPLY gate is ACCEPTED from durable exact-run evidence. The next ChatAdmin step is the distinct protected rollback-only synthetic DB/RLS ACCEPT under `FM-CHATADMIN-OWNER-ACCEPT-20260926`, after a fresh current-main/target binding. Do not repeat APPLY. The real manual ChatAdmin application flow remains later.
