...
2. **Mobile — FM-MOB-001, R3:** signed Android preview and the bounded real-device UI/runtime observation are complete for FM-MOB-003/FM-MOB-004. PR #1028 merged the Android `1.0.0` Store control as `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`; its exact signed AAB is published in the closed Google Play Alpha track for Germany, Austria and Switzerland, and `fanmind://reset-password` is saved in the exact Production Auth allowlist. FM-MOB-006 accepted the isolated-Staging atomic Push Delivery-Ledger gate on exact commit `18a6ad79cb72331b4daa41ee87dd2430a8ffd473`: apply `33867831888` / `101006621418` and rollback-only acceptance `33867922978` / `101006906941` passed with provider sending disabled, synthetic rows, complete rollback and cleanup. Run `33868661986` / job `101009217307` then queued exactly one handler-containing signed Android `preview` build for commit `700885307c265f8907cefe5f5b10499a5ea7b996`; GitHub timed out fail-closed while it waited, but authenticated read-only EAS inspection later proved that the same internal build finished with version `1.0.0 (2)` and an APK. No retry, Submit or OTA Update occurred. Installation/device observation and separately authorized real provider delivery remain open; the missing automated receipt means the complete private 19-check acceptance is not yet claimable from this build. The cohort of at least 12 opted-in testers for at least 14 days stays deferred until Gerhard handoff readiness. `iOS-TestFlight` remains Phase 8.
3. **AI/Billing — FM-AI-001, R3:** FM-EV-022 currently proves the synthetic Staging resource, Plus/Ultra and complete five-price Test catalog, exact 22-event webhook, installed empty AI ledger and 50/100/150 context policy. FM-EV-033 accepts the bounded Stripe SDK/payment-method/outbound-version correction: PR #1035 final head `ffdc11ab4a1c199134dc009abc516cc8257f5e8b` passed all eight exact-head workflows and completed review with zero unresolved threads, then merged as `9a7b37f2cee798dc64c1d32f70fda338db174b5e`. The separate provider-side inbound webhook migration, product/private evidence, unapplied general Billing ledger/cutover, current post-ledger lifecycle, Legal/Tax, runtime integration and explicit activation remain open.
...
10. **Phase 8 Website AI — FM-WEB-001/FM-WEB-002/FM-WEB-003, R3:** repository-side Website Chat now includes a dormant consent-bound human email handoff, complete existing CRM timeline linkage, a checksum-pinned atomic processing gate, separate exact-main protected handoff controls and a second checksum-pinned dry-run-first retention contract. It is `IMPLEMENTED_NOT_VERIFIED`: no protected workflow was dispatched, both controlled SQL files are unapplied, all installations remain disabled and real Staging acceptance, retention apply/acceptance/scheduling, AI dialogue, uncertainty escalation, email verification and approved delivery remain open. The retention contract deletes only eligible technical session/evidence rows after Handoff expiry and explicitly preserves CRM contacts, Conversations and messages.
...
3. **FM-MOB-001:** preserve the accepted bounded UI/runtime result and published Alpha AAB. The atomic Push Delivery-Ledger isolated-Staging gate is accepted on `18a6ad79cb72331b4daa41ee87dd2430a8ffd473`; exactly one later handler-containing signed Android Preview for commit `700885307c265f8907cefe5f5b10499a5ea7b996` is now verified finished at EAS with an APK. Do not requeue it. Next install that existing artifact for bounded device observation; keep any real provider delivery separately authorized. When FanMind is ready for the Gerhard handoff, enroll at least 12 approved testers for at least 14 days and complete the private Android/Recovery proof before requesting Production access. iOS signing/build/TestFlight/device work remains Phase 8.
