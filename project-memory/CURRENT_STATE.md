...
- #1137 is merged/deployed as `67773a794936ba48100d811e21358239d72e26b3`. The subsequent real Facebook connect attempt exposed a Production config false-positive: the OAuth URL used literal repository placeholders for App ID and redirect URI and Meta returned “Ungültige App-ID”. Zero Facebook/Instagram connections remain on the Admin-CRM workspace.
- Immediate source fix: placeholders/examples must never count as configured; Facebook OAuth readiness requires a numeric App ID, usable secret, HTTPS FanMind callback path and token encryption. Invalid config disables the button and never leaves FanMind. Actual protected Meta Production credentials remain an owner/provider action and must not be disclosed in chat.
- After valid Meta configuration, expected flow is Meta consent -> optional explicit Page selection -> FanMind callback -> saved Page connection -> bounded DM import. Facebook comments are a separate permission/import path; both messages and comments must ultimately enter FanMind. Payment and Mobile remain deferred.
...
1. **FM-SOC3-001:** FM-CR-044 / PR #1137 is the current executable repository priority. Reuse the accepted Facebook/Instagram OAuth, Page/account binding, encrypted-token and bounded first-import/sync foundations; finish current-head CI/review and deploy, then hand the existing granted account to `/channels` for Facebook first and Instagram second. Real provider consent/App Review/permissions/webhook/inbound evidence remains external.
...
9. **FM-META-001:** real App Review/provider/legal evidence stays separate. Sales Handoff still requires the genuine remaining gate quorum and final Production demo.
