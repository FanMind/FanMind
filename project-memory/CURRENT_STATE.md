## ChatAdmin source merged; Staging release-state recovery in progress — 2026-09-26
...
- Protected Staging deploy `36247833422` stopped before build/restart on the ambient-environment Creator test. PR #1194 fixed it, passed all triggered checks and review, and merged as `8ebb344633618de27c570734210c41ce69fc7721`. Runtime reread at `2026-09-26T14:16:38.983Z` remained the old release.
- Corrected-source deploy `36248407036` / job `108421775888` then stopped at `STAGING_BILLING_CAPTURE=FAIL`. The prior source copy deletes `.release.env` and `.next/` before tests; executable reproduction confirms the failure mode. Current bounded branch `fix/staging-release-state-preservation-20260926` preserves these paths and prepares exact live-service recovery of only missing nonsecret release metadata with unchanged Billing values/proofs. Review/CI/merge and actual protected recovery/deploy remain open; no capture/Stripe action is authorized or executed.
