...
- #1137 is merged/deployed as `67773a794936ba48100d811e21358239d72e26b3`; owner then performed the real Facebook connect attempt with the existing granted account.
- New browser evidence: FanMind incorrectly reported Facebook server configuration ready although the OAuth URL contained literal placeholders `replace_with_facebook_app_id` and `your-domain.example`; Meta rejected it as “Ungültige App-ID”. No valid Page connection or message import occurred.
- Current source correction: reject placeholder/example OAuth values, use only validated dedicated/legacy fallback configuration, require callback + token encryption for readiness, and keep the connect button disabled until the effective configuration is usable. Also clarify that Messenger messages and Page comments are both intended ingestion paths, with separate comment permission.
- Retained provider evidence from #1121: authenticated central FanMind Meta app inspection on 2026-09-14; Instagram Messaging/Content use case was later saved after explicit owner consent. These do not prove current live connectivity.
- Still open: install/revalidate actual Production Meta app ID/secret/callback in the protected server environment, current Facebook/Instagram consent, permissions/App Review/Advanced Access as applicable, webhook publication/verification and real inbound/import/isolation/duplicate/revocation/reconnect proof.
- Exact next step: publish/review/deploy the fail-closed config fix, then obtain the real protected Meta Production configuration without exposing secrets. Re-test the same account: Facebook -> official Meta consent/Page selection -> return to FanMind -> connected status -> bounded initial DM import -> comments permission/import -> Instagram analog.
