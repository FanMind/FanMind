...
- Dependencies: Supabase recovery redirect and applicable Push/Store evidence; Android real-device acceptance is complete, while iOS/TestFlight is Phase 8.
...
- Exact next step: keep the accepted Android UI/runtime result closed and continue only through the separate recovery/Push/Store external controls. Do not queue another build merely to repeat acceptance.
- Owner action needed: the separate recovery/Push/Store external controls only.
