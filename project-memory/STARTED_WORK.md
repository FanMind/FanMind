...
- Exact next step: owner browser acceptance only: locate the confirmed registration in Platform Admin, grant permanent free CRM access, then verify Web login/Dashboard/Fans/Inbox/Follow-ups. No payment or Social-provider activation in this step.
- Owner browser evidence 2026-09-19: registration, confirmation and permanent Admin grant succeeded, but the granted account is incorrectly routed to `/billing/start`. Source/deploy/database rollout are complete; FM-CR-043 now fixes the paid-routing fallthrough. Do not repeat the grant. Owner retest is required only after hotfix deployment.
