...
- Exact next step: PR #1082 merged as `205febde8ef62f632e488337a6e887d1abb532f9`, but delayed review findings invalidated its acceptance. PR #1085 is the active repository-only repair; exact-head review through `11f7baa5df0cc11ef75dd93282b281d93fe9337c` additionally required an exclusive invocation/recovery lock, durable rollback replacement of the write reservation, and terminal receipt publication only after all blocking markers are removed. Those paths are now implemented locally; renew exact-head CI/review before acceptance. A real external Storage transition remains deferred under `FM-RST-OWNER-007`; never rerun the database Restore and never infer `STORAGE_RESTORED` from the synthetic proof.
