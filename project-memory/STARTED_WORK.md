...
- Exact next step: PR #1082 merged as `205febde8ef62f632e488337a6e887d1abb532f9`, but five delayed review findings arrived after merge. Repair the controller and add regressions before treating the repository-only controller as accepted. A real external Storage transition remains deferred under `FM-RST-OWNER-007`; never rerun the database Restore and never infer `STORAGE_RESTORED` from the synthetic proof.
