...
- Repository result: the Full-Backup verifier can now prepare only the validated Storage part as a private non-overwriting archive plus receipt. Exact extracted path-set equality, per-object size/hash, aggregate count/size, unsafe/duplicate tar members and receipt/artifact/commit/part bindings fail closed. The separate validator rechecks the private pair without provider access.
- Exact next step: merge the repository preparation after exact-head CI/review, then obtain a new exact R4 authorization for a dedicated isolated non-Production Storage controller/target. Never rerun the database Restore.
- Owner action needed: authorize the later isolated Storage mutation only after exact target/project, exact accepted Full Backup, exact main commit, rollback/cleanup plan and Production/Supabase-Staging exclusions are bound. No current authorization permits upload.
## FM-MOB-001
