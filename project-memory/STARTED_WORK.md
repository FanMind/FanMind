...
- Still open: reconcile exact receipt coverage for `DB_RESTORED -> DB_POSTCHECKED`. Storage, server config, disposable-target cleanup, independent countercheck and final aggregate acceptance remain open unless separate evidence proves them. #1075/#944 and the database Restore itself are closed.
...
- Exact next step: determine whether existing private receipts fully satisfy every `DB_POSTCHECKED` predicate; use read-only proof for any gap. Then continue with the first genuinely unproven later transition. Never rerun the database Restore.
- Owner action needed: none for the closed #1075/#944 correction or read-only evidence reconciliation. Any later Storage/server-config/cleanup mutation requires its own current scope and authorization.
## FM-MOB-001
