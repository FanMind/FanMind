...
- Exact next step: PR #1082 merged as `205febde8ef62f632e488337a6e887d1abb532f9`, but delayed review findings invalidated its acceptance. PR #1085 is the active repository-only repair; exact-head review through `50b72792bc72bd1c3961ef222e1c23641f676cc3` additionally required lock-replacement precedence, safe post-rename sync failure, local-only failure selection after proven rollback and corrected lock-file documentation. Those paths are now implemented locally; renew exact-head CI/review before acceptance. A real external Storage transition remains deferred under `FM-RST-OWNER-007`; never rerun the database Restore and never infer `STORAGE_RESTORED` from the synthetic proof.
