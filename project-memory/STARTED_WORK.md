...
- Completed so far: initial implementation/docs and local checks passed. Three successive reviewed heads each passed all nine CI gates and were deliberately not merged because their reviews found six, four and six valid cases. The first two corrections added provenance, semantic statuses/counting and fail-closed Mobile/legacy behavior. Review three then required priority selection before the 1.000-row cap, stable `created_at`/`id` page boundaries, section-specific Dashboard/knowledge errors, Web provenance rendering and proof that all provenance columns—not only one—are absent before legacy fallback. Those six fixes are grouped without schema, row, provider or Production mutation.
- Still open: publish the fully revalidated replacement exact head, resolve the six newest threads, require all replacement exact-head gates, merge, one Android preview and owner device confirmation.
