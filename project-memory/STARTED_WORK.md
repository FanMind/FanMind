...
- Branch/PR: permanent target-principal projection correction `fix/restore-target-principal-projection-20260907` / #1075; earlier SSH/extension/fail-closed reconciliation remains historical in PRs #1005/#997/#995.
...
- Still open: merge the permanent helper correction in #1075, then reconcile the exact post-database state-machine boundary. Storage, server config, disposable-target cleanup, independent countercheck and final aggregate acceptance remain open unless separate evidence proves them. A new database Restore is not open.
...
- Exact next step: finish exact-head CI/review and merge #1075; then update issue #944 and determine the first genuinely unproven post-database state-machine transition. Never rerun the database Restore to close the helper bug.
- Owner action needed: none for the repository-only #1075 correction. Any later Storage/server-config/cleanup mutation requires its own current scope and authorization.
## FM-MOB-001
