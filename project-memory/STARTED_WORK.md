...
- Exact next step: PR #1082 merged as `205febde8ef62f632e488337a6e887d1abb532f9`, but delayed review findings invalidated its acceptance. PR #1085 is the active repository-only repair; exact-head review through `fae7ab69a9db9b1340135acb4cd3d341f9b89fc8` additionally required ownership-aware recovery of an indeterminate operation marker and pending/reservation replacement precedence over the ordinary lock. Those paths are now implemented locally; renew exact-head CI/review before acceptance. A real external Storage transition remains deferred under `FM-RST-OWNER-007`; never rerun the database Restore and never infer `STORAGE_RESTORED` from the synthetic proof.
