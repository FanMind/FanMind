...
- Exact next step: correct the generated PROJECT_STATUS.md drift in draft PR #1121, then resume the specific Meta setup once the remaining approval boundary is resolved.
- Owner authorization / remaining block: Bernd explicitly answered yes to adding the named Instagram Messaging/Content use case. One subsequent save was again rejected by automatic approval review, now because the bundled case includes content publishing as well as messaging. The original authorization is retained; the reviewer requires explicit acknowledgment of that broader bundle. Selection remains unsaved. The existing FanMind messaging OAuth path only requests basic/manage_messages, and no source code requests content_publish. No narrower selectable Meta setup was established; do not retry the rejected save indirectly.
