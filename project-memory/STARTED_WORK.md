...
- Work lock: LOCK-FINISHLINE-RESUME-20260906 remains ACTIVE only for FM-AI-001/FM-MOB-001; FM-RST-001 was removed after the database phase closed. Repository-only postcheck reconciliation uses `LOCK-FM-RST-001-DB-POSTCHECK-RECONCILIATION-20260907`.
...
- Exact next step: build and countercheck the missing bounded Storage restore/verification path without dispatching it. Any isolated Storage mutation requires a new exact authorization; never rerun the database Restore.
