...
- Historical limitation at this 2026-08-26 snapshot: the successful historical transactional lifecycle run `31735315959` predated the applied AI ledger and used legacy service-role CRUD; AI ledger apply run `32038152382` occurred later. At this observation the separately controlled general Billing ledger tables/functions were absent and its workflow had never run. **That ledger-absence/workflow-never-run claim is superseded by FM-EV-035 and the 2026-09-06 Billing installation/capture/canonical rollback evidence. Do not use this historical limitation to repeat the completed Billing Staging sub-gate.** Remaining provider-side current lifecycle/downstream reconciliation evidence is still open.
...
- Stripe conformance observations at the 2026-08-26 read-only snapshot: the internal daily-test Checkout fixed card-only `payment_method_types`, the endpoint was pinned to `2026-06-24.dahlia`, and server calls used raw REST rather than the current client pattern. Accepted FM-EV-033 supersedes the productive code observations through PR #1035 and merge `9a7b37f2cee798dc64c1d32f70fda338db174b5e`; the provider endpoint observation remains current and separately gated. No automatic provider change is implied.
- Negative evidence: write flags stayed false/blank, no SQL Apply or transactional acceptance ran during this 2026-08-26 snapshot, and no Stripe product/price/webhook/payment/refund, Supabase row/schema, paid-tier, Production, Restore, Mobile or Security mutation occurred. Focused local policies passed 175/175 and all offline control checks passed.
- Classification: synthetic resource, Test prices, webhook configuration and AI-ledger installation are read-only `PASS` for this snapshot; FM-EV-035 supplies the later bounded Billing Staging acceptance. Overall FM-AI-001 remains `PARTIAL` and is deferred to `FM-AI-OWNER-001`/the narrowed `002` plus private/external evidence.
- Falsification: any later catalog drift, ledger schema/ACL drift, nonzero unresolved reconciliation, changed price/webhook contract, or lifecycle run inconsistent with the current evidence reopens reconciliation before activation.
...
- Result: `mobile:device:acceptance:prepare` initially validated a private mode-0600 redacted Preview receipt, rejected duplicate or drifted receipt fields and created exactly one mode-0600 pending-only evidence template bound to the receipt hash, exact commit and build timestamp. It refused overwrite, unsafe directories, invalid normalized calendar timestamps and any pre-passed check or safety observation. `GOOGLE_PLAY_HANDOFF.md` bound the remaining portal sequence to the already verified Android `1.0.0` AAB and prohibited a new build. The focused Mobile set passed 29/29, Store readiness passed, the full operations suite passed 1062/1062, focused ESLint passed and Product Truth/Project Memory/drift controls passed. FM-EV-031 subsequently extends the schema to bind the exact Android Production/Store receipt after installation from Play; the earlier Preview-only boundary is historical, not the current Store procedure.
...
- Limitations: the 19 real-device checks, real Recovery flow, actual Store screenshots/legal confirmation, Google phone verification/app record/test-track/AAB upload and Play review remain external. Google developer identity review is still pending.
- Falsification: any generated template that exposes receipt values, pre-passes a check, accepts an unsupported receipt class, overwrites evidence, is not bound to the exact receipt or survives a failed validator invalidates this preparation. For the current Store path, anything other than the existing Android `production`/`store` receipt is invalid. A changed Google status requires fresh portal reconciliation before continuing.
...
- Date: 2026-08-30
- Target: PR #1031 final head `a963ab598eeb0a7ab84110e55cb4043d4230e550`, merge/Production commit `3082490451dd45b5127bdf9d9ae55b4712255b72`; existing Android AAB commit `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00`; no EAS, Google Play or Apple portal write target
...
- Result: the repository now contains a reproducible 32-bit RGBA 512×512 Google Play icon and language-neutral 24-bit RGB 1024×500 feature graphic from existing FanMind vector branding, a public `/support` route, complete iPhone-only App Store metadata/screenshot/review handoff, reusable-review-account safety guidance and a conditional Play tester plan. Store readiness verifies both graphics, the separate HTTPS Support URL and mandatory Google Play support e-mail, separate length-checked DE/EN Apple keyword sets, Apple metadata lengths, 6.9-inch `1320 × 2868` screenshot plan, iPhone-only identity and explicit no-iOS-build/TestFlight boundary. Sharp is declared and locked inside the isolated Mobile package, and SVG source hashing normalizes line endings. Device-evidence schema version 2 accepts only the historical Preview/Internal class or Android Production/Store class and binds environment, receipt SHA, exact commit, platform, profile, distribution and timestamp. The current Android handoff and canonical architecture require the exact Production/Store receipt from `33316172583` / `99269924756` and begin the complete 19-check/Recovery/screenshot acceptance only after download from the Play test track; the old Preview receipt cannot certify that run. All 14 exact-head check runs on PR #1031 completed successfully before merge; post-merge deploy, Browser E2E, Supply Chain, CodeQL, Final Go-Live Readiness and read-only Production Audit succeeded on exact `3082490451dd45b5127bdf9d9ae55b4712255b72`. The live `/support` page returned the required contact and recovery/account-deletion/privacy guidance.
...
- Limitations: Google identity review still blocks phone verification/app creation. Play-track install/device acceptance, Apple Developer/App Store Connect, signed iOS build, TestFlight, actual screenshots, accessibility/age/privacy portal answers and real-device evidence remain external.
...
- Acceptance: ACCEPTED for the repository preparation and deployed public Support scope only; external Android/Apple Store controls remain OPEN

## FM-EV-032
- Related: FM-MEM-009 / FM-CR-008 / FM-LOOP-011 / issues #642/#643/#644/#874
- Date: 2026-08-30
- Target: repository-only legacy issue reconciliation on `ops/legacy-issue-reconciliation-20260830` plus bounded post-merge GitHub issue metadata; no runtime/provider/database/billing/Mobile build target
- Type: immutable workflow-run crosscheck + machine-readable checkbox map + deterministic human rendering + fail-closed negative tests
- Result: all 23 formerly unchecked #642 items, 9 formerly unchecked #643 items and 14 formerly unchecked #644 items are classified as accepted, partial, retained, transferred or procedural-superseded. #642/#643 retain explicit genuine gates; #644 is eligible for deliberate supersession by #874. The accepted Staging milestone and current successful runs `31837057323`/`31895476403` are bound to exact commits.
- Checks: 10/10 focused reconciliation tests and 1073/1073 full Operations tests passed; Product Truth across 253 files, Referral truth, legal evidence schema, immutable Action policy, focused ESLint, Project Memory Quality/NBA/V8/drift/freshness/milestone/status and diff whitespace checks passed locally.
- Negative evidence: the validator rejects unknown or mutated evidence references/commits, missing retained gates, an open #644 target, missing or count-coordinated historical checkbox mappings, coordinated status/evidence/gate reclassification and machine/human rendering drift. The ordered historical source lists and each complete ordered classification tuple are bound to independently fixed SHA-256 digests; evidence records remain exact immutable contracts. No issue is closed merely because later umbrella evidence exists; no Production/Stripe/database/provider/payment/signing/build/legal action is authorized.
- Exact-head/merge evidence: PR #1033 final head `70ea1bc61c7adefb739ba8fa3e16ea0bb84b4e58` passed all 11 checks and completed review with zero unresolved threads, then squash-merged as `cc82dd7ad62e6aaf1d7b2637d49d43010789475f`.
- Post-merge evidence: exact-main deploy run `33332571325`, Browser E2E `33332571190`, Supply Chain `33332571180`, CodeQL `33332571188`, public readiness `33332629364` and installed Production read-only verification `33332629383` all completed successfully.
- Issue metadata evidence: independent post-write reads on 2026-08-30 confirmed #642/#643 open, #644 closed with reason `not_planned` as superseded by #874, and #874 open with the verified `e96415035ffbe12f16dd3b81e13a5e62b2c4ac00` Android AAB reuse/no-new-build rule plus Google-pending and Phase-8 iOS boundaries.
- Freshness boundary: mutable issue-state snapshot `EV-LEGACY-ISSUE-SNAPSHOT-20260830` records each issue's observation-time `updated_at`, state/reason and body SHA-256. Every GitHub Actions Project Memory Quality run reads #642/#643/#644/#874 through a read-only token and fails when body, state or state reason drifts; comment-, label- or assignment-only timestamp changes do not block unrelated work. A separate seven-day TTL reminder is scoped to `legacy_issue_reconciliation`, not the immutable V6 acceptance gate; revalidation is also mandatory before relying on or reopening reconciled work.
- Limitations: the retained #642/#643/#874 gates are not accepted by this governance closeout.
- Falsification: a current issue body with a historical unchecked item absent from the map, a referenced run not successful on its bound commit, #642/#643 closed without their retained evidence, #644 remaining an active execution source after the closeout, or generated/rendered drift invalidates acceptance.
- Acceptance: ACCEPTED

## FM-EV-033
- Related: FM-AI-001 / FM-CR-009 / FM-LOOP-004
- Date: 2026-08-30
- Target: repository-only Stripe runtime conformance on PR #1035 final head `ffdc11ab4a1c199134dc009abc516cc8257f5e8b`, merged as `9a7b37f2cee798dc64c1d32f70fda338db174b5e`; no Stripe/provider, payment, database, tax-registration, tier-activation, Production-config or Mobile target
- Type: SDK/API contract implementation + runtime-format check + source-level fail-closed regression + complete local repository countercheck
- Result: all four productive server-side Stripe call sites now use one cached Stripe Node SDK `22.4.0` client pinned to outbound API `2026-07-29.dahlia`, two bounded automatic network retries and a 12-second timeout. Checkout no longer sends `payment_method_types`; eligible methods are managed by Stripe. Every Session receives `fanmind_checkout_` plus a cryptographically selected eight-letter suffix. Invoice reads prefer the current `total_taxes` response while retaining bounded transition compatibility for `total_tax_amounts`; checkout create/expire/reconcile, period-end cancellation and Referral coupon/subscription operations use typed SDK resources; a coupon-creation race is accepted only after the resulting coupon is re-read and matches the exact Starter-Core contract.
- Version boundary: the separately observed isolated Staging webhook endpoint remains verified and checked at inbound `2026-06-24.dahlia`. This code change does not rewrite or claim migration of that provider resource; migration and fresh signed/lifecycle acceptance remain owner/protected work under FM-AI-OWNER-002.
- Checks: Stripe conformance 5/5; focused Billing/Referral/cancellation 47/47; full Operations 1074/1074; TypeScript, focused ESLint, Next Production build, dependency audit, Product/Referral/Legal/Actions truth, Project Memory Quality/NBA/freshness/drift/status and diff whitespace passed. The final exact head passed Project Memory Guard `33336153230`, Status `33336153349`, Quality `33336153377`, Landing `33336153631`, FanMind CI `33336153351`, Supply Chain `33336153455`, CodeQL `33336153327` and Browser E2E `33336153341`; final Codex review completed with no new suggestion and all three earlier threads were resolved. Root dependency audit remains zero findings; the pre-existing time-bounded Mobile-only review exception is unchanged.
- Negative evidence: no Stripe API or Dashboard call, payment/refund, price/coupon/subscription mutation, webhook endpoint update, SQL/database write, protected workflow, tax-registration claim, Plus/Ultra activation, Production configuration, Mobile build, Submit or Store action occurred.
- Limitations: FM-AI-001 stays PARTIAL; a newer outbound SDK version is not evidence that the older inbound provider endpoint has migrated, and the merge does not authorize provider/product/lifecycle activation.
...
- Date: 2026-08-30
- Target: repository-only App Store Connect preparation on PR #1037 final head `88b9299f9612e344a9c0c48d78f86f11d071db6c`, squash-merged as `a16e28f6e1aa0a2d7ff81bd679b472fab7563500`; no Apple portal, EAS, signing, TestFlight, device, Google Play, database, provider or Production target
...
- Checks: Store readiness passes with `33/13/12/8`; focused Mobile Store/native/boundary tests pass 36/36; Mobile TypeScript, Expo Doctor 20/20, boundary and Android/iOS native prebuild pass; full Operations pass 1074/1074; root ESLint and clean Next Production build pass; Product/Referral/Legal/Actions truth plus Project Memory Quality/NBA/V8/freshness/drift/milestone/status pass. The final exact head passed all eight workflows: Project Memory Status `33339507449`, Guard `33339507459`, Quality `33339507448`, Landing `33339507454`, Mobile `33339507457`, FanMind CI `33339507450`, Browser E2E `33339507465` and CodeQL `33339507444`. Final Codex review completed for `88b9299` with no major issue and zero unresolved threads.
...
- Acceptance: ACCEPTED for the bounded repository/App Store Connect preparation; external Apple account/legal/portal, signed-binary and device controls remain OPEN

## FM-EV-035
- Related: FM-AI-001 / FM-CR-022 / EXT-AI-STAGING-LIFECYCLE
- Date: 2026-09-06
- Target: isolated FanMind Staging on exact `main` `62e6a11858e85996af03f6740819b0fc6194b4a4`; no Production or live Stripe target
- Type: controlled general Billing ledger installation/capture history + exact isolated-Staging deployment + rollback-only canonical Billing acceptance + independent read-only Supabase countercheck
- Reference: general Billing Apply `34040107219`; durable signed capture `34043010578`; unfreeze `34043148548`; `Deploy FanMind Staging` run `34058028839`; `FanMind Canonical Billing Staging Acceptance` run `34058118450`, job `101553652111`; `project-memory/receipts/FM-AI-001-CANONICAL-BILLING-STAGING-ACCEPTED-20260906.md`.
- Result: the general Billing ledger is installed on isolated Staging. The exact deployment and rollback-only canonical acceptance passed on the same reviewed commit. Shared rollout returned `STAGING_DATABASE_ROLLOUT_STATE=PASS` and `STAGING_DATABASE_ROLLOUT_STRIPE_BILLING_LEDGER=verify`; Billing postflight returned `PASS`, cutover pending `0` and uninventoried `0`; the canonical fixture returned `PASS`; both postflight/canonical transactions rolled back and cleanup returned `PASS`.
- Independent countercheck: after acceptance, read-only Staging state remained unchanged at one previously durable capture event, zero Billing reconciliations/streams, zero AI entitlement/event/reconciliation rows and zero Mobile Push registrations. This is consistent with complete rollback and no accidental runtime/provider activation.
- Supersession: FM-EV-022 and `AI_BILLING_READONLY_RECONCILIATION_2026-08-26.md` remain valid historical 2026-08-26 snapshots, but their old statements that the general Billing ledger was absent/unapplied or its workflow had never run are superseded by this evidence. Do not repeat the completed general Billing Apply/capture/canonical Staging acceptance merely to resume work.
- Negative evidence: no Production database write, no live Stripe/provider call, no real payment/refund, no canonical Production runtime projection activation, no Plus/Ultra activation, no Mobile provider send and no Restore mutation occurred.
- Limitations: this closes only the bounded technical canonical Billing Staging sub-gate. `FM-AI-001` and `ai_billing` remain `PARTIAL`; `sales_ready=false`; Plus/Ultra and canonical Production projection remain disabled. Product/private quality-cost, provider-side inbound webhook/current lifecycle and downstream AI/referral reconciliation, Legal/Tax, Production runtime integration and explicit activation remain open.
- Falsification: a later exact log/target read showing missing/drifted ledger state, nonzero unresolved cutover/reconciliation state, incomplete rollback, unexpected provider/live-mode activity or Production projection activation invalidates reliance on this bounded acceptance.
