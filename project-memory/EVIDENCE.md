...
- Stripe conformance observations at the 2026-08-26 read-only snapshot: the internal daily-test Checkout fixed card-only `payment_method_types`, the endpoint was pinned to `2026-06-24.dahlia`, and server calls used raw REST rather than the current client pattern. FM-EV-033 now supersedes the productive code observations pending exact-head closeout; the provider endpoint observation remains current and separately gated. No automatic provider change is implied.
...
- Date: 2026-08-30
- Target: repository-only Stripe runtime conformance on `feat/stripe-client-conformance-20260830`; no Stripe/provider, payment, database, tax-registration, tier-activation, Production-config or Mobile target
...
- Limitations: exact-head PR checks, independent review, merge and repository closeout remain pending. FM-AI-001 stays PARTIAL; a newer outbound SDK version is not evidence that the older inbound provider endpoint has migrated.
...
- Acceptance: COUNTERCHECKED_LOCALLY_PENDING_EXACT_HEAD
