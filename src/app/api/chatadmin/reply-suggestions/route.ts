...
export async function POST(request:NextRequest){
 ...
 let workspaceId="",userId="",inputChars=0;const model=getFanMindAiModel(),started=Date.now();
 try{
  ...
  await recordAiUsageEvent({workspaceId,userId,feature:"chat_admin_reply",model,inputChars,outputChars:(result.replies as string[]).join("").length,status:"ok",latencyMs:Date.now()-started,sourceRoute:"/api/chatadmin/reply-suggestions",providerUsage:payload.usage});
