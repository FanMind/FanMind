...
type Status = { available: boolean; connected: boolean; accountName: string | null; expiresAt: string | null; nextReadAt: string | null };
...
export function SocialProviderConnection({ provider, demo }: { provider: "tiktok" | "x"; demo: boolean }) {
  ...
  async function perform(action: "start" | "disconnect" | "messages") {
    ...
    } catch { setNotice("Die Aktion konnte nicht abgeschlossen werden. Bitte prüfe den Verbindungsstatus."); }
  ...
  return <section className={styles.releaseBox} aria-label={`${name} verbinden`}>
    ...
    <p>{provider === "tiktok"
      ...
      : "Die offizielle Anmeldung bereitet das Lesen deiner Direktnachrichten vor. Im freigegebenen Pilot kannst du bis zu 20 aktuelle Ereignisse prüfen."}</p>
