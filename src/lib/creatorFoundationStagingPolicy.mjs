...
export const CREATOR_FOUNDATION_VERIFY_CONFIRMATION =
...
function evaluateCommitBinding(environment, errors) {
  ...
  const reviewedCommit = clean(
    environment.FANMIND_CREATOR_FOUNDATION_REVIEWED_COMMIT,
  ).toLowerCase();
...
export function evaluateCreatorFoundationStagingEnvironment(
  environment = {},
  { mode = "verify" } = {},
) {
  if (!MODES.has(mode)) {
  ...
  if (clean(environment.FANMIND_CREATOR_INTELLIGENCE_ENABLED) === "true") errors.push("runtime_must_remain_disabled");
