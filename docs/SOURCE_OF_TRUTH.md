...
- Daily (0 € Setup + 1 €/Tag) ist gemäß FM-DEC-019 eine manuell vom Platform-Admin gesteuerte interne Beta, täglich kündbar und ohne Referral-Rabatt. FM-DEC-020 legt den vom Kunden bezahlten Daily-Gesamtpreis auf **1,00 € brutto pro Tag einschließlich der jeweils anwendbaren Umsatzsteuer** fest. Die bestehende Engine samt `internal_daily_test` und Stripe-Preis bleibt kompatibel erhalten. Es gibt keinen automatischen Countdown. Bei ausgeschalteter Beta ist Daily öffentlich unsichtbar und für neue Provisionierung/Checkouts gesperrt; bestehende Daily-Abos laufen unverändert weiter. Authentifizierte Workspace-Provisionierung und Checkout benötigen weiterhin sichere RPCs, Browser-INSERT-Verbot, aktuelle ausdrückliche Zustimmung sowie vollständige Tax-/Stripe-/Webhook-/Billing-Readiness. Die historische Beta-Fensterlogik und die Preisentscheidung sind kein Ersatz für diese Freigaben; generische Migrationen bleiben ausgeschlossen.
...
- Starter-Beträge bleiben nach dem bisherigen Stand Nettopreise. Abweichend davon ist Daily gemäß FM-DEC-020 ein Endpreis von 1,00 € brutto pro Tag einschließlich der jeweils anwendbaren Umsatzsteuer. Der Owner hat Rumänien als Verkaufsland genannt; die tatsächliche Verkäufergesellschaft, Steuerregistrierung und Stripe-Tax-Konfiguration sind damit nicht belegt und bleiben vor jeder Aktivierung extern zu bestätigen. Bis dahin bleiben alle kostenpflichtigen Checkouts fail-closed.
