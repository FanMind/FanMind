...
as $function$
  select
    ...
    workspace.billing_status::text,
    workspace.workspace_access_mode::text,
...
-- Creator RPCs retain browser execution; their own canonical dynamic gate
-- delegates to admin_crm_read_allowed whenever both controlled contracts exist.
...
begin
  ...
  -- The per-user advisory lock plus this in-transaction check prevents a
  -- concurrent provisioning path from adding a second Workspace membership.
  if exists (
    select 1
      from public.workspace_members as membership
     where membership.user_id = p_target_user_id
       and (v_workspace_id is null or membership.workspace_id <> v_workspace_id)
  ) then
    raise exception using errcode = '23514', message = 'admin_crm_access_existing_membership';
  end if;
