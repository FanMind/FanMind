...
create table public.social_provider_connections (
  ...
  initial_read_pending boolean not null default false,
  lease_id uuid,
...
begin
  ...
  insert into public.social_provider_connections(workspace_id,provider,external_account_id,display_name,encrypted_token,expires_at,initial_read_pending)
  values(p_workspace,p_provider,p_account,p_name,p_token,p_expires,p_provider='x')
  on conflict(workspace_id,provider) do update set display_name=excluded.display_name,encrypted_token=excluded.encrypted_token,
    ...
    initial_read_pending=excluded.initial_read_pending;
  return true;
end $$;
...
create function public.fanmind_social_claim_read(p_workspace uuid,p_user uuid,p_provider text,p_revision uuid,p_lease uuid,p_initial_only boolean default false)
returns setof public.social_provider_connections language plpgsql security invoker set search_path = '' as $$
begin
  if not public.fanmind_social_owner(p_workspace,p_user) or p_provider<>'x' or p_lease is null then return; end if;
  return query update public.social_provider_connections set lease_id=p_lease,lease_until=now()+interval '2 minutes',next_read_at=now()+interval '15 minutes',initial_read_pending=false
  where workspace_id=p_workspace and provider=p_provider and revision=p_revision and next_read_at<=now()
    and (lease_until is null or lease_until<now()) and (not p_initial_only or initial_read_pending) returning *;
end $$;
...
revoke all on function public.fanmind_social_owner(uuid,uuid),
 ...
 public.fanmind_social_claim_read(uuid,uuid,text,uuid,uuid,boolean),public.fanmind_social_rotate(uuid,uuid,text,uuid,uuid,text,timestamptz),
