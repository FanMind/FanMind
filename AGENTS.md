...
- FM-DEC-018 confirms provider-hosted login/consent and return to the selected channel; never collect platform passwords. Central FanMind apps are configured once, each Creator authorizes their own account. The initial X preview requires its atomically consumed database permission, never a callback query. Its separate schema workflow stays reviewed-main-, protected-Staging- and exact-target-bound with checksum/pre/postflight and no runtime activation. The historical Backup failure remains open separately; independent Social source work continues without weakening Operations acceptance.
...
- Current commercial truth: the paid Pilot/Setup package is retired. Owner decision FM-DEC-014 approves the permanent public offers `Starter Flex = 990 € one-time setup + 312 €/month` and `Starter 12 Monate = 0 € setup + 312 €/month with a 12-month minimum term, then monthly renewal`. Owner decision FM-DEC-019 changes Daily back to a manually controlled internal beta: 0 EUR setup + 1 EUR/day, daily cancellable, no referral discount, no automatic countdown. Reuse its existing internal_daily_test engine/Price for compatibility. A Platform Admin may enable new Daily admission only after full readiness and may disable it at any time; when disabled, Daily is absent from public surfaces and new admission/checkout, while existing Daily subscriptions continue unchanged. Public account selection does not bypass current consent, tax, Workspace or billing readiness. Core includes one creator/workspace, KI Standard and 10 connections. Each additional five connections are planned at +49 €/month. KI Plus is +100 €/month; KI Ultra is +200 €/month. Agency remains Coming Soon: self-paying creators are managed without double billing; agency-paid creators use a 312 €/month Hub plus 0/5/10/15 % volume tiers. Referral applies only to Core and cannot be combined with an Agency volume discount. Do not reintroduce the old Pilot or 299 €/month pricing.
...
- Active Mobile Phase B repository scope: independent Expo/React-Native app with native login, PKCE password recovery, secure session persistence and local purge, dashboard, contact list/search/create/edit/detail, bounded encrypted offline contact overview, contact knowledge, server-side AI reply suggestions, user-controlled copy plus native sharing of only the selected reply text, follow-ups, an explicit opt-in push registration and a dormant Staging-only server-delivery contract with a controlled, unapplied delivery ledger but without route, timer, provider send or activation, a native wordmark splashscreen, distinct high-resolution iOS/Android app-icon assets, an iOS required-reason privacy manifest and Android API 36 verification. Store privacy drafts are technical preparation only; Supabase redirect approval, signed internal builds, push migration/secret activation, protected delivery-ledger Staging apply/acceptance, legal/portal approval, real-device verification and store distribution remain separate external release steps.
