...
function fixture(paid=false){
 const snapshot={workspaceId:'11111111-1111-4111-8111-111111111111',customerId:'cus_owner',subscriptionId:'sub_base',requestId:'req_one',observedAt:'2026-09-06T16:00:00.000Z',status:'active',cancelAtPeriodEnd:false,basePriceId:'price_base',items:[{id:'si_base',priceId:'price_base',start:1788624000,end:1791216000},...(paid?[{id:'si_plus',priceId:'price_plus',start:1788624000,end:1791216000}]:[])]};
...
test('missing canonical base price fails closed before AI projection',()=>{const f=fixture(true);delete f.observation.snapshot.basePriceId;f.observation.fingerprint=hash(f.observation.snapshot);f.billingCommand.providerSnapshotFingerprint=f.observation.fingerprint;const r=plan(f);assert.equal(r.reason,'ai_price_configuration_invalid');});
...
test('rotation requires exact mismatch evidence but remains blocked until the workspace base binding can move atomically',()=>{
 const f=fixture(true);f.inventory.current={...current(f,'reconciliation_needed'),stripe_subscription_id:'sub_previous'};
 const evidence={...conflict,reason:'subscription_mismatch',subscriptionId:f.inventory.subscriptionId,workspaceId:f.inventory.workspaceId,customerId:f.inventory.customerId};
 f.inventory.unresolvedEvents=[evidence];
 let result=plan(f);assert.equal(result.status,'blocked');assert.equal(result.reason,'ai_rotation_binding_unavailable');assert.equal(result.rpcBody,undefined);
 for(const patch of [{reason:'other'},{subscriptionId:'sub_other'},{customerId:'cus_other'},{workspaceId:'22222222-2222-4222-8222-222222222222'}]){f.inventory.unresolvedEvents=[{...evidence,...patch}];result=plan(f);assert.equal(result.reason,'ai_rotation_evidence_missing');}
