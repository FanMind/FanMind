...
const receipt = (body,component) => ({component,status:'reconciled',workspaceId:body.p_workspace_id,requestId:body.p_stripe_request_id,snapshotFingerprint:body.p_snapshot_fingerprint});
function harness() { const calls=[]; return {calls, input:fixture(), environment, reviewedTarget:target, clock:()=>now, adapters:{reconcileAi:async b=>{calls.push('ai');return receipt(b,'ai');},reconcileReferral:async b=>{calls.push('referral');return receipt(b,'referral');},commitBilling:async b=>{calls.push('billing');return [{result_status:'reconciled',result_revision:b.p_expected_revision+1}];}}}; }
...
test('expired snapshot after downstream work cannot activate billing',async()=>{const h=harness();let reads=0;h.clock=()=>now+(reads++ ? 900001:0);assert.equal((await execute(h)).reason,'snapshot_expired');assert.deepEqual(h.calls,['ai','referral']);});
