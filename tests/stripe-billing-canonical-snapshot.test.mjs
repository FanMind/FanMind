...
const subscription=()=>({id:'sub_base',object:'subscription',customer:'cus_owner',livemode:false,status:'active',metadata:{workspace_id:target.workspaceId},pending_update:null,pause_collection:null,cancel_at:null,cancel_at_period_end:false,items:{object:'list',has_more:false,data:[{id:'si_base',subscription:'sub_base',quantity:1,current_period_start:1788624000,current_period_end:1791216000,price:{id:'price_base',object:'price',livemode:false}}]},latest_invoice:{object:'invoice',id:'in_paid',customer:'cus_owner',livemode:false,status:'paid',amount_remaining:0,parent:{subscription_details:{subscription:'sub_base'}}},lastResponse:{requestId:'req_snapshot'}});
...
test('binding, pending updates, partial items and unpaid invoices fail closed',async()=>{
 for(const patch of [{customer:'cus_other'},{metadata:{workspace_id:'other'}},{pending_update:{}},{pause_collection:{}},{status:'trialing'},{items:{...subscription().items,has_more:true}},{latest_invoice:{...subscription().latest_invoice,status:'open'}},{latest_invoice:'in_not_expanded'}]){const h=harness();h.stripe.subscriptions.retrieve=async()=>({...subscription(),...patch});assert.equal((await read(h)).status,'blocked');}
});
