...
- Öffentliche Registrierung: kostenloses Anmeldekonto mit E-Mail-Bestätigung und erneutem Versand; Paket/Referral werden nur vorgemerkt, ohne Workspace, Zahlungszustimmung oder Abo. Zwei dauerhafte öffentliche Zahlungsmodelle: Starter Flex (990 € Setup + 312 €/Monat) und Starter 12 Monate (0 € Setup + 312 €/Monat). Die manuell gesteuerte interne Daily-Beta (0 € Setup + 1 €/Tag) ist nur während der Admin-Freigabe sichtbar. Nach der Anmeldung folgt die geschützte Einrichtung mit frischer Paketwahl und Zustimmung (`docs/operations/WEB_REGISTRATION.md`). Kostenpflichtige Aktivierung benötigt weiterhin die bestätigte Vertragsversion, passende Datenbankfunktionen, Tax- und Billing-Freigabe.
- Kostenlose Demo: temporärer, geschützter Demo-Workspace; kein entgeltliches Pilot-Paket.
- Daily: manuell vom Platform-Admin gesteuerte interne Beta, täglich kündbar und von Referral ausgeschlossen. Der bestehende technische Bezeichner `internal_daily_test`, der 1-€/Tag-Stripe-Preis und die Billing-Engine werden wiederverwendet. Der vom Kunden bezahlte Daily-Gesamtpreis ist gemäß FM-DEC-020 **1,00 € brutto pro Tag einschließlich der jeweils anwendbaren Umsatzsteuer**. Es gibt keinen automatischen Countdown. Bei ausgeschalteter Beta ist Daily öffentlich unsichtbar und für neue Provisionierung/Checkouts gesperrt; bestehende Daily-Abos laufen unverändert weiter. Sichere Workspace-Provisionierung, Browser-INSERT-Verbot, Stripe/Webhook/Tax und aktuelle Zahlungszustimmung bleiben Pflicht. `docs/operations/INTERNAL_DAILY_TEST_WORKSPACE_PROVISIONING.md` beschreibt den weiter erforderlichen kontrollierten Datenbankvertrag.
- Billing-Steuermodus: Starter-Beträge bleiben nach dem bisherigen Stand Nettopreise; Daily ist die ausdrücklich beschlossene Brutto-Ausnahme. `FANMIND_TAX_MODE=stripe_tax` plus die getrennte Registrierungsbestätigung sind Pflicht; ohne beides bleibt Checkout fail-closed. Der Owner nennt Rumänien als Verkaufsland, hat damit aber weder die tatsächliche Verkäufergesellschaft noch eine Steuerregistrierung oder Stripe-Tax-Konfiguration belegt. Die externe steuerliche Prüfung bleibt offen.
...
| `/register` | Kostenloses Anmeldekonto | E-Mail bestätigen; kostenpflichtige Aktivierung bleibt separat gesperrt |
| `/register/confirm` | E-Mail-Bestätigung | Geprüfte Identität, bewusste Anmeldung, Weiterleitung zur bestehenden Einrichtung |
...
| Daily | interne, manuell ein-/ausschaltbare Beta; derzeitige Aktivierungsabnahme offen | 0 € Setup + 1 €/Tag; täglich kündbar; vorhandener Billing-Lifecycle, keine Referral-Verrechnung; Workspace-/Consent-/Tax-/Billing-Voraussetzungen bleiben erforderlich |
