...
// Superseded pre-review checksums retained as historical regression markers:
// 0138a2a8484b526f8064abb45f6f0026174c38717e3bf04fc484f9dcb3a2624c
// 498f6fc91c46023fe0d38b38e05c8db452f41aca0f9939ff8a86b9a334e7add9
const EXPECTED_SHA256 = "ff59c2b6fe81277eb3c3faad0084ff0f27d9cf81b390d81e737c7d2997aa8fe6";
...
for (const pattern of [
  ...
  /create or replace function public\.guard_processing_account_deletion_workspace_billing/u,
  /old\.stripe_subscription_id is distinct from new\.stripe_subscription_id[\s\S]*old\.subscription_effective_end_at is distinct from new\.subscription_effective_end_at[\s\S]*old\.billing_status is distinct from new\.billing_status/u,
  /new\.id = any\(r\.owned_workspace_ids\)/u,
  /create trigger guard_processing_account_deletion_workspace_billing[\s\S]*before update of stripe_subscription_id, subscription_effective_end_at, billing_status[\s\S]*on public\.workspaces/u,
  /create or replace function public\.guard_processing_account_deletion_workspace_members/u,
  /tg_op = 'INSERT'[\s\S]*new\.workspace_id = any\(r\.owned_workspace_ids\)[\s\S]*new\.user_id is distinct from r\.user_id/u,
  /tg_op = 'DELETE'[\s\S]*old\.workspace_id = any\(r\.owned_workspace_ids\)[\s\S]*old\.user_id is distinct from r\.user_id/u,
  /create trigger guard_processing_account_deletion_workspace_members[\s\S]*before insert or delete or update of workspace_id, user_id on public\.workspace_members/u,
  ...
  /if v_request\.status = 'processing'[\s\S]*v_request\.owned_workspace_ids is null[\s\S]*lock table public\.workspaces in share mode[\s\S]*lock table public\.workspace_members in share mode[\s\S]*v_owned_workspace_ids is distinct from v_request\.owned_workspace_ids[\s\S]*message = 'workspace_inventory_drift'[\s\S]*wm\.workspace_id = any\(v_request\.owned_workspace_ids\)[\s\S]*w\.id = any\(v_request\.owned_workspace_ids\)[\s\S]*message = 'processing_blocker_drift'[\s\S]*owned_workspace_ids := v_request\.owned_workspace_ids[\s\S]*return next/u,
  ...
  /revoke all on function public\.guard_processing_account_deletion_workspace_billing\(\)[\s\S]*from public, anon, authenticated/u,
  /revoke all on function public\.guard_processing_account_deletion_workspace_members\(\)[\s\S]*from public, anon, authenticated/u,
